...
r: integer;
  ...
  begin
    pEngineCore.ConsoleWrite(PAnsiChar(IntToStr(r)));
  end;
begin
  check(pEngineCore.GetSubsystem( ESS_RESOURCE_MANAGER, IEngineSubSystem( pResMan ) ));
  check(pEngineCore.GetSubsystem( ESS_RENDER, IEngineSubSystem( pRender ) ));
  check(pEngineCore.GetSubsystem( ESS_INPUT,  IEngineSubSystem( pInput ) ));
  check(pRender.GetRender2D( pRender2D ));
  // Background clear color setup.
	check(pRender.SetClearColor(Color4(38, 38, 55, 255)));
  // This example adapted only for 800X600 resolution, if even resolution will be higher it won't effect it.
	check(pRender2D.SetResolutionCorrection(GAME_VP_WIDTH, GAME_VP_HEIGHT));
  pInput.GetType(r);
  check(r);
  prender.GetType(r);
  check(r);
  ...
  check(pInput.Configure( ICF_HIDE_CURSOR ));

  //The way we can get default system font
  pResMan.GetDefaultResource( EOT_BITMAP_FONT, IEngBaseObj( pFont ) );

  pResMan.Load(RESOURCE_PATH + 'sounds\owl.wav', IEngBaseObj(pSndOwl));
  pResMan.Load(RESOURCE_PATH + 'sounds\forest_ambient.wav', IEngBaseObj(pForestAmbient));

  pForestAmbient.PlayEx(pChannelAmbientLoop, SSP_LOOPED);


  pResMan.Load( RESOURCE_PATH + 'sprites\cartoon_forest_background.png',    IEngBaseObj( pBg ),     TEXTURE_LOAD_DEFAULT_2D );
  pResMan.Load( RESOURCE_PATH + 'sprites\cartoon_cloudy_night_sky.jpg',   IEngBaseObj( pSky ),    TEXTURE_LOAD_DEFAULT_2D );
  pResMan.Load( RESOURCE_PATH + 'textures\smoke.png',   IEngBaseObj( pFog ),    TEXTURE_LOAD_DEFAULT_2D );
  pResMan.Load( RESOURCE_PATH + 'textures\light.jpg', IEngBaseObj( pLightRound ), TEXTURE_LOAD_DEFAULT_2D );

  pResMan.Load( RESOURCE_PATH + 'sprites\cartoon_anime_girl.png', IEngBaseObj( pTexGirl ), TEXTURE_LOAD_DEFAULT_2D );
  pTexGirl.SetFrameSize( 55, 117 );

  pResMan.Load( RESOURCE_PATH + 'sprites\cartoon_mistery_light.jpg', IEngBaseObj( pLight ), TEXTURE_LOAD_DEFAULT_2D );
  pLight.SetFrameSize( 64, 128 );

  pResMan.Load( RESOURCE_PATH + 'sprites\vox.jpg', IEngBaseObj( pVox ), TEXTURE_LOAD_DEFAULT_2D );
  pVox.SetFrameSize( 149, 149);

  pResMan.Load( RESOURCE_PATH + 'sprites\cartoon_owl.png', IEngBaseObj( pOwl ), TEXTURE_LOAD_DEFAULT_2D );
...
begin
  ...
  pRender2D.DrawTex( pSky, Point2(), Point2(GAME_VP_WIDTH, GAME_VP_HEIGHT) );
  ...
  pRender2D.DrawTexCropped( pBg, Point2( -200.0, 150.0 ), Point2( 1399.0, 517.0 ), RectF( 0, 905, 1399, 517 ), 0, EF_BLEND );
  ...
  if owlGoLeft then
	...
	pRender2D.DrawTexSprite(pOwl, Point2(owlX, 425.0), Point2(48.0, 128.0), (counter div 3) mod 15, 0.0, EF_BLEND or EF_COLORMIX or OwlEffFlag);

  // Draw tree

  pRender2D.DrawTexCropped( pBg, Point2(), Point2( 1399.0, 900.0 ), RectF( 0, 0, 1399, 900 ), 0, EF_BLEND );
  ...
  pRender2D.DrawTex( pLightRound, Point2( Lights[0].X - 32.0, Lights[0].Y + 64.0 ), Point2( 256.0, 256.0 ), 0, ( EF_VERTICES_OFFSET or EF_BLEND or EF_COLORMIX ) );
  ...
  pRender2D.DrawTexSprite( pTexGirl, Point2( 550.0, 725.0 ), Point2( 60.0, 120.0 ), ( Counter div 5 ) mod 16, 0, ( EF_VERTICES_OFFSET or EF_BLEND or EF_COLORMIX ) );

  //Girl Sprite
  pRender2D.DrawTexSprite( pTexGirl, Point2( 550.0, 725.0), Point2( 60.0, 120.0 ), ( Counter div 5 ) mod 16, 0, EF_BLEND );

  //Lights
  pRender2D.SetBlendMode( EBF_ADD );
  for i := 0 to 4 do
    pRender2D.DrawTexSprite( pLight, Point2( Lights[i].X, lights[i].Y ), Point2( 64.0, 128.0 ), ( Counter div 2 ) mod 14, 0, EF_BLEND );
  ...
  pRender2D.DrawTexSprite(pVox, Point2(pos.x - 37, pos.y - 37), Point2(75.0, 75.0), (counter div 2) mod 16, 0.0, EF_BLEND);
